...
export const motivoRepiqueService = {
  ...
  async exportMotivosToExcel() {
    ...
    const rows = await motivoRepiqueRepository.findAll();
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Motivos de Repique');
  ...
  async bulkCreateMotivos(file, actorId, { dryRun = false } = {}) {
    ...
    const existentes = nombres.length ? await motivoRepiqueRepository.findByNombres(nombres) : [];
    ...
    if (!dryRun && filasUnicas.length > 0) {
      await motivoRepiqueRepository.bulkUpsert(
        filasUnicas.map((f) => ({
          nombre: f.nombre,
